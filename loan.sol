...
// You can edit this contract as much as you want. A template is provided here and you can change the function names and implement anything else you want, but the basic tasks mentioned here should be accomplished.
    ...
    function getCompoundInterest(uint256 principle, uint rate, uint time) public pure returns(uint256) {
        ...
        // https://medium.com/coinmon4db014e080b1ks/math-in-solidity-part-3-percents-and-proportions- just read the towards full proportion part.
        ...
        for(uint i=0;i<time;i++){
          principle+=mulDiv(principle,rate,100);
        }
  ...
  Request(msg.sender,principle,rate,time,toPay);
  if(balances[Owner]>toPay){
    ...
    function viewDues()  isOwner() view public returns (uint){

     return loans[Owner];
    }
    function settleDues() isOwner() public returns(bool){
  // address indexed receivers=0x1aE0EA34a72D944a8C7603FfB3eC30a6669E454C;
      if(loans[Owner]<=balances[Owner]){
          sendCoin(msg.sender,loans[Owner],Owner);
          loans[Owner]=0;
          return true;
      }else{
return false;
